 ...
 * @author Asus
 */
public class Innings1Controller implements Initializable {
    ...
    ObservableList<String> teamList1;
    ObservableList<String> teamList2;
    ...
    int batSelected = 0;
    int bowlSelected = 0;
    public static ArrayList<Player> team1Bat = new ArrayList<Player>();
    public static ArrayList<Player> team2Bowl = new ArrayList<Player>();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            ...
            strike = 1;
            extras = 0;
            if(team1.size()>0) team1.clear();
            if(team2.size()>0) team2.clear();
            if(team1Bat.size()>0) team1Bat.clear();
            if(team2Bowl.size()>0) team2Bowl.clear();
            ...
            rs.next();
                ...
                team1.add(rs.getString("captain"));
                team1.add(rs.getString("player1"));
                team1.add(rs.getString("player2"));
                team1.add(rs.getString("player3"));
                team1.add(rs.getString("player4"));
                team1.add(rs.getString("player5"));
                team1.add(rs.getString("player6"));
                team1.add(rs.getString("player7"));
                team1.add(rs.getString("player8"));
                team1.add(rs.getString("player9"));
                team1.add(rs.getString("player10"));
                team1.add(rs.getString("player11"));
                team1.add(rs.getString("player12"));
            stat.close();
            con.close();
                
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/crictrack", "root", "");
            stat = con.createStatement();
            query = "SELECT `captain`, `player1`, `player2`, `player3`, `player4`, `player5`, `player6`, `player7`, `player8`, `player9`, `player10`, `player11`, `player12` FROM `" + CricTrack.usernAme + "` WHERE `teamname` = '" + NewMatchController._team2 + "'";
            rs = stat.executeQuery(query);
            rs.next();
            
                team2.add(rs.getString("captain"));
                team2.add(rs.getString("player1"));
                team2.add(rs.getString("player2"));
                team2.add(rs.getString("player3"));
                team2.add(rs.getString("player4"));
                team2.add(rs.getString("player5"));
                team2.add(rs.getString("player6"));
                team2.add(rs.getString("player7"));
                team2.add(rs.getString("player8"));
                team2.add(rs.getString("player9"));
                team2.add(rs.getString("player10"));
                team2.add(rs.getString("player11"));
                team2.add(rs.getString("player12"));
            stat.close();
            con.close();
            
            
            teamList1 = FXCollections.observableArrayList();
            teamList2 = FXCollections.observableArrayList();
            ...
            bat1.getItems().clear();
            bat1.setItems(teamList1);
            bowl.getItems().clear();
            bowl.setItems(teamList2);
            ...
            tossOrTarget.setText("Toss won by " + NewMatchController.tossWin);
            if(NewMatchController.opt == 1) optOrScore.setText("opted to bat first");
            else optOrScore.setText("opted to bowl first");
            innings1Or2.setText(NewMatchController._team1 + " , 1st innings");
            ...
            Logger.getLogger(Innings1Controller.class.getName()).log(Level.SEVERE, null, ex);
        }
    }    
    public void setALL(){
    ...
    private void selectBat(ActionEvent event) {
        if(bat1.getValue() != null && batSelected<2){
            if(batsman1 == null){
                batsman1 = bat1.getValue();
                bat1.setValue(null);
                batSelected++;
                team1.remove(new String(batsman1));
                teamList1 = FXCollections.observableArrayList();
                //teamList1.clear();
                ...
                batsman2 = bat1.getValue();
                bat1.setValue(null);
                batSelected++;
                team1.remove(new String(batsman2));
                teamList1 = FXCollections.observableArrayList();
                //teamList1.clear();
    ...
    private void selectBowl(ActionEvent event) {
        if(bowl.getValue() != null){
            if(bowler == null){
                ...
                team2.remove(new String(bowler));
                teamList2 = FXCollections.observableArrayList();
                //teamList2.clear();
    ...
    private void zero(ActionEvent event) throws IOException {
        if(batSelected==2 && bowler!=null){
            ...
            if(totalBall<BallsCanPlay && totalWicket<10){
                int x  = totalBall/6;
                int y = totalBall % 6;
                ...
                team2Bowl.add(obj3);
                Parent window1;
                window1 = FXMLLoader.load(getClass().getResource("mid.fxml"));
